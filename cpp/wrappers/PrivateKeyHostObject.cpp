...
const PrivateKey& PrivateKeyHostObject::getPrivateKey() const { return *privateKey; }
        ...
        return jsi::Function::createFromHostFunction(runtime, jsi::PropNameID::forAscii(runtime, funcName), 2,
            [this](jsi::Runtime& runtime, const jsi::Value& thisValue, const jsi::Value* arguments,
                ...
                if (!isTypedArray(runtime, object)) {
                    throw jsi::JSError(
                        runtime, "The 'fromBytes' argument must be an object, but it is not of type Uint8Array.");
                }
                ...
                if (typedArray.size(runtime) != PrivateKey::PRIVATE_KEY_SIZE) {
                    throw jsi::JSError(runtime,
                        "Invalid size for 'fromBytes' argument. Expected "
                            + std::to_string(PrivateKey::PRIVATE_KEY_SIZE) + " bytes.");
                }
