...
class JsiAugSchemeMPL : public RNBls::JsiBlsHostObject {
public:
  //----------------------------skToG1----------------------------//
  ...
  JSI_HOST_FUNCTION(keyGen) {
    ...
    PrivateKey sk = AugSchemeMPL().KeyGen(Utils::ArrayBufferToVector(arrayBuffer, runtime));
  ...
  explicit JsiAugSchemeMPL() : RNBls::JsiBlsHostObject() {}
};
