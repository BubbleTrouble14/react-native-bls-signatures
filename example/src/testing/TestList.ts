    ...
    registrator: () => {
      ...
      describe('Invalid vectors', () => {
        ...
        for (const [i, input] of invalidInputs1.entries()) {
          ...
          it(`G1 element ${i}`, () =>
            expect(() => assert(G1Element.fromBytes(bytes))).to.throw());
        }
        for (const [i, input] of invalidInputs2.entries()) {
          const bytes = fromHex(input);
          it(`G2 element ${i}`, () =>
            expect(() => assert(G2Element.fromBytes(bytes))).to.throw());
        }
      });
      ...
      describe('Readme', () => {
        ...
        describe('PopSchemeMPL prove', () => {
          it('First pop verify', () =>
            assert(PopSchemeMPL.popVerify(pk1, pop1)));
          it('Second pop verify', () =>
            assert(PopSchemeMPL.popVerify(pk2, pop2)));
          it('Third pop verify', () =>
            assert(PopSchemeMPL.popVerify(pk3, pop3)));
        });
